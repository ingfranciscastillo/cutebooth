...
function drawSparkle(ctx: CanvasRenderingContext2D, x: number, y: number, s: number) {
  ctx.beginPath();
  ctx.moveTo(x, y - s);
...
function drawDecor(ctx: CanvasRenderingContext2D, theme: StripTheme) {
  ...
  for (let i = 0; i < 26; i++) {
    const left = i % 2 === 0;
    const x = left ? rand(i) * (PAD - 8) + 5 : W - PAD + 3 + rand(i + 40) * (PAD - 9);
...
function drawCover(
  ...
  const sw =
    (src as HTMLCanvasElement).width ?? (src as HTMLVideoElement).videoWidth ?? w;
  const sh =
    (src as HTMLCanvasElement).height ?? (src as HTMLVideoElement).videoHeight ?? h;
...
export function renderStrip({ photos, theme, caption, scale = 2 }: StripOptions) {
