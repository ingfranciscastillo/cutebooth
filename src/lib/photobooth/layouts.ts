...
function vertical(n: number): Layout {
  ...
  const cells: Cell[] = [];
  for (let i = 0; i < n; i++) cells.push({ x: PAD, y: HEADER + i * (ph + GAP), w: pw, h: ph });
...
function horizontal(n: number): Layout {
  ...
  for (let i = 0; i < n; i++) {
    ...
    cells.push({ x: PAD + c * (pw + GAP), y: HEADER + r * (ph + GAP), w: pw, h: ph });
  }
...
function square(n: number): Layout {
  ...
  for (let i = 0; i < n; i++) {
    const c = i % cols;
    const r = Math.floor(i / cols);
    cells.push({ x: M + c * (pw + GAP), y: TOP + r * (ph + GAP), w: pw, h: ph });
  }
  const footTop = TOP + rows * ph + (rows - 1) * GAP;
  return {
    format: "square",
    width: S,
    height: S,
